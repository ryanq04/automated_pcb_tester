...
def get_frame(ser: Serial, rows: int, cols: int, suffix: bytes = b"") -> np.array:
    """
    Get a frame from the serial port, handle RGB data.
    """
    rx_len = rows * cols * 3 + len(suffix)  # 3 bytes per pixel (RGB)
    max_tries = 1000
    raw_img = b""
    ...
    raw_img = raw_img[: rows * cols * 3]  # Take only the number of bytes needed (RGB)

    # Convert raw bytes to a NumPy array with 3 channels (RGB)
    img_rgb = np.frombuffer(raw_img, dtype=np.uint8).reshape((rows, cols, 3))
